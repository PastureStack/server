# PastureStack Server v1.6.323

This release retains the complete `v1.6.322` authentication, account,
authorization, database, catalog, runtime, Orchestration Engine, and vSphere
CLI changes while replacing the embedded Web Console with the reviewed `.35`
artifact.

## Runtime change

- Base image: `ghcr.io/pasturestack/server:v1.6.319`
- Runtime image: `ghcr.io/pasturestack/server:v1.6.323`
...
- Web Console: `1.6.56-pasturestack.35`
...
## Authentication experience and browser icon

The authentication shell now uses a responsive PastureStack-owned visual,
consistent full-width actions, safe spacing below the primary action, and a
restrained animated gradient. Local credentials, OpenID Connect, SAML, GitHub,
MFA, and recovery continue to use the existing authentication behavior. The
layout includes explicit form labels and browser autocomplete contracts, plus
mobile, short-window, right-to-left, reduced-motion, and forced-color handling.

The static artifact includes both SVG and ICO browser icons. Server assembly
requires a non-empty root `favicon.ico` and verifies that the generated index
references both icon formats, so the standard browser icon request cannot
silently regress to a 404.
...
YAML sources, and browser acceptance covered English and Traditional Chinese,
localized credential failure, reduced motion, horizontal overflow, and the
...
- The formal endpoint must remain on `v1.6.322` until an isolated `v1.6.323`
