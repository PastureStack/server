...
separate SSH client remains absent. Git 2.53.0 is newer than the affected
versions listed for CVE-2024-52005. The unused `git-http-push` program is
removed together with Expat, its sole Expat-linked runtime path, because the
upstream CVE-2025-66382 denial of service remains unfixed. The only coreutils
binary rebuilt from upstream source is `uniq`; it is GNU coreutils 9.11 with
upstream fix commit
