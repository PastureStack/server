...
The `v1.6.277` assembly consumes Orchestration Engine `v0.183.269`, Web Console `1.6.56`, API Explorer `1.1.14`, Compose Executor `v0.14.31`, Node Agent `v1.2.31`, Load Balancer Service `v0.9.25`, Catalog Service `v0.20.7`, and the Catalog at commit `91f5910a44cb181051be2adc4c14f0e6ec7842ef`. Catalog Service rebuilds an empty index even when the recorded repository commit is unchanged. Web Console packaging must retain its fingerprinted `/assets/ui*.js` entry, and API Explorer must retain `/api-ui/ui.min.js` and `/api-ui/ui.min.css`.
