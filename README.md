...
`v1.6.416` combines Orchestration Engine `0.183.295`, Node Agent `0.13.27`,
...
Server `v1.6.416` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.106` preserves the Server `v1.6.358` authenticated visual and
...
First-time service creation now keeps the saved service through link persistence
and route completion, so a successful create navigates exactly once instead of
leaving the form visible with a stale-model error that invites duplicate
submissions. Empty link sets no longer invoke a redundant API action, and the
route retains its immutable stack ID if a non-empty link update returns a
partial resource. The same guarded completion applies to virtual-machine, load
balancer, external-service, and alias creation routes. Advanced key/value inputs
also show the localized value hint instead of an untranslated key.
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.416
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.416
...
Server `v1.6.416` also keeps the embedded-database startup context explicit, so
