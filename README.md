...
`v1.6.372` combines Orchestration Engine `0.183.286`, Node Agent `0.13.22`,
...
Server `v1.6.372` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.75` restores the global dropdown destination required by the
maintained PowerSelect runtime, so environment and user selectors render their
actual options. It also preserves the hidden-state compatibility contract, so
inactive full-screen overlays cannot intercept navigation or filter controls,
and prevents the legacy positioning shim from throwing on Bootstrap 5 events.
It retains the audit-log filter builder, existing audit table, Bootstrap runtime
...
pins Node.js `24.19.0` and npm `12.0.2`, and fails closed when the current
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.372
```
