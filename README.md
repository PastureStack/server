...
This is a compatibility-focused modernization project. Existing Ubuntu 26.04, Java 25, MariaDB, modern Docker, non-root runtime, artifact-integrity, authentication, WebSocket, backup/restore, and test work is retained. Server `v1.6.352` combines Orchestration Engine `0.183.281`, Node Agent `0.13.22`, and the reviewed Ember 6.12 LTS Web Console `1.6.56-pasturestack.61` to provide authoritative host-port and volume conflict preflight. Managed-network preflight checks every eligible host in the environment even when a target host is selected; any active owner blocks the create or upgrade. Bridge and host-network checks remain scoped to the selected host, stopped owners produce an explicit warning, and unknown live-inspection state is never reported as safe. Start-first upgrades reserve the current service mappings without reporting the service as its own conflict; changed mappings remain blocked when old and new tasks cannot coexist. Project authorization exposes the nested preflight input schemas and read-only result schemas, allowing the Web Console to call the action without weakening resource ownership checks. The Engine also registers every volume-preflight schema in the runtime core type set; image gates reject a package that contains authorization text but cannot resolve the live action schema. The UI identifies the affected environment, stack, service, scale, batch size, start-first policy, requested mapping, container, host, state, protocol, bind address, and endpoint when available. Closure actions used by port preflight are invoked directly, absent optional callbacks are safe, and serialized port changes use a named component action that remains valid on Ember 6.12 LTS. Host storage page-size controls keep writable state in the route controller and receive semantic `All` selection through an explicit callback without writing through a caller-owned input. They clamp the current page after live row removal and refresh visible rows, pagination, selection counts, and controls after each successful selected-volume removal. A directly opened or reloaded host-container route follows the host resource's native `instances` relationship and passes that exact collection to the table. Shared sortable tables refresh when an API relationship is populated, when its collection reference is replaced, and when initial attributes are received. Natural sorting, pagination, column selection, search, and live statistics remain synchronized. The release retains readable WCAG AA Catalog documentation code blocks, reactive Catalog upgrade version choices, localized questions, writable OpenID Connect provider configuration, broker-aware terminal recovery, the single-owner project WebSocket reconnect fix, API Explorer `1.1.15`, administrator-controlled MFA, and all 13 reviewed production locales. SMTP delivery is configured once per installation by a system administrator; individual accounts store only their own verified recovery destination address. Passing the documented compatibility gates does not by itself make a deployment production-ready.
...
Server `v1.6.352` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.352
```
