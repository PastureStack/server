...
`v1.6.379` combines Orchestration Engine `0.183.286`, Node Agent `0.13.22`,
...
Server `v1.6.379` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.82` preserves the Server `v1.6.358` authenticated visual and
...
the audit-log builder without changing its result table. Server `v1.6.358` is used only as visual
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.379
```
