...
`v1.6.417` combines Orchestration Engine `0.183.295`, Node Agent `0.13.27`,
...
Server `v1.6.417` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.107` preserves the Server `v1.6.358` authenticated visual and
...
First-time service creation now keeps the saved service only as completion data
through link persistence. Route completion uses the immutable stack query input
and never reads routing fields from a partial or unreadable saved response, so a
successful create navigates exactly once instead of leaving the form visible
with a stale-model error that invites duplicate submissions. Empty link sets no
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.417
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.417
...
Server `v1.6.417` also keeps the embedded-database startup context explicit, so
