...
`v1.6.374` combines Orchestration Engine `0.183.286`, Node Agent `0.13.22`,
...
Server `v1.6.374` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.77` restores the classic locale observer contract required by
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.374
```
