...
`v1.6.418` combines Orchestration Engine `0.183.295`, Node Agent `0.13.27`,
...
Server `v1.6.418` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.108` preserves the Server `v1.6.358` authenticated visual and
...
First-time service creation now invokes and awaits the template's closure
callback after persistence instead of routing completion through deprecated
`sendAction` dispatch. The saved service remains completion data through link
persistence, while route selection continues to use the immutable stack query
input. A successful create therefore leaves the form exactly once instead of
showing an `undefined.get` error that invites duplicate submissions; create and
upgrade use the same completion contract. Empty link sets no
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.418
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.418
...
Server `v1.6.418` also keeps the embedded-database startup context explicit, so
