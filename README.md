...
`v1.6.419` combines Orchestration Engine `0.183.295`, Node Agent `0.13.27`,
...
Server `v1.6.419` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.109` preserves the Server `v1.6.358` authenticated visual and
...
First-time service creation and service upgrade now pass classic named
completion actions through the shared form so Ember preserves the controller
receiver after persistence. The legacy compatibility layer also refuses to
forward a component prototype callback as an action name. The saved service
remains completion data through link persistence, while route selection
continues to use the immutable stack query input. A successful operation can
therefore leave the form exactly once without the `undefined.get` or invalid
template-action error that invites duplicate submissions. Empty link sets no
longer invoke a redundant API action. The same stable query-input routing applies
to virtual-machine, load-balancer, external-service, and alias creation routes.
...
`02df5f7df9eebe640590d93b1506543d2367e355`. It retains prior immutable
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.419
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.419
...
Server `v1.6.419` also keeps the embedded-database startup context explicit, so
