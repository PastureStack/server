...
`v1.6.469` combines Orchestration Engine `0.183.322`, Node Agent `0.13.27`,
...
Post-release multi-account validation showed that `v1.6.452` fixed the stale
provider-state check but did not complete the token boundary: after resolving
the platform account, the Engine adds its own stable `rancher_id` and then
mistook that internal identity for another external OIDC identity. `v1.6.453`
packages Orchestration Engine `0.183.312`, which validates every provider
identity before account lookup or mutation, then handles only the
Engine-generated stable identity that resolves to the authenticated account.
A provider-supplied, mismatched, missing, or unknown identity still fails
closed. See [v1.6.453 release notes](docs/releases/server-1.6.453.md).

The first fresh-user runtime matrix against `v1.6.453` then exposed a separate
account-lifecycle race. External account creation scheduled `account.create`
in the background, while the token path immediately entered MFA; MFA correctly
rejected the transient `registering` account as inactive. `v1.6.454` packages
Orchestration Engine `0.183.313`, which completes that lifecycle synchronously
...
`v1.6.456` fixes repeated OpenID Connect login resolving to the built-in
`token` account on upgraded installations. Orchestration Engine `0.183.315`
persists authentication credentials for the explicitly verified user,
excludes internal accounts from login-identity lookup, and repairs a legacy
misowned link only when the built-in owner, provider, identity type, external
ID, link digest, and target user all match. Every other ownership conflict
...
`v1.6.460` reconciles successful local and OpenID Connect logins into the
...
replaced. Engine `0.183.318` additionally resolves an existing OIDC account
before restricted-site authorization, so a returning user with an existing
project membership is admitted without adding that stable membership to
...
`v1.6.462` closes the shared-Default authorization race and the remaining
zero-environment and OIDC account-presentation gaps. Engine `0.183.319`
performs the role-preserving membership decision and insert under one project
lock, so a concurrent administrator update cannot be weakened by a later
baseline member grant. Web Console `1.6.126` treats an authenticated account
with no environment as a supported empty state, clears stale project scope,
uses authoritative identity links with a local fallback, and keeps those links
out of account update payloads. The release matrix compares complete v1 and
v2-beta project and stack sets for direct-user and OIDC-group roles, verifies a
real zero-environment browser flow, edits an account through the real modal,
and confirms that display-only identity data is not serialized. See
[v1.6.462 release notes](docs/releases/server-1.6.462.md).

`v1.6.463` uses Engine `0.183.320` to check project-member collection
requests against the requested project before loading members. A token scoped
to one environment cannot read another environment's memberships by combining
an authorized `X-API-Project-Id` header with a different `projectId` query.
Web Console remains at `1.6.126`. See
[v1.6.463 release notes](docs/releases/server-1.6.463.md).

`v1.6.464` keeps the Engine authorization boundary and updates Web Console
`1.6.127`. Environment view and edit failures distinguish missing or forbidden
project and member data from a server failure. The save form explains when
only part of an edit may have been stored. Identity search separates a
...
`v1.6.467` resumes interrupted account-network cleanup and rejects
development-only Engine files from the release archive. The Web Console shows
clearer account, security, and stack access errors without treating a denied
identity lookup as empty data. These changes do not make every API endpoint
equivalent: the permission matrix records tested paths and remaining gaps
separately. See [v1.6.467 release notes](docs/releases/server-1.6.467.md).

`v1.6.468` corrects the administrative account page's identity lookup scope:
project accounts in the general account collection are not user identities,
and their expected 404 must not block the user/admin list. Permission errors
for visible accounts remain visible. See
[v1.6.468 release notes](docs/releases/server-1.6.468.md).

`v1.6.469` fixes the environment editor's network-policy lookup: it now sends
the selected project context required by the scoped network API. The same
project context was already used for network saves and policy-manager queries.
See [v1.6.469 release notes](docs/releases/server-1.6.469.md).
...
Web Console `1.6.132` preserves the Server `v1.6.358` authenticated visual and
...
Workload creation and upgrade controls now use the same effective-schema
permission decision as their direct routes. Catalog management also requires
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.469
```
