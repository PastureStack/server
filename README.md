...
`v1.6.385` combines Orchestration Engine `0.183.286`, Node Agent `0.13.22`,
...
Server `v1.6.385` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.88` preserves the Server `v1.6.358` authenticated visual and
...
that is sent to the API. The result table's final authentication/IP heading now
wraps inside its original cell in longer locales instead of being clipped;
result rows, column order, and other page regions are unchanged.
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.385
```
