...
`v1.6.421` combines Orchestration Engine `0.183.295`, Node Agent `0.13.27`,
...
Server `v1.6.421` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.111` preserves the Server `v1.6.358` authenticated visual and
...
First-time service creation and service upgrade now pass receiver-bound
completion callbacks through the shared form. The four top-level container and
virtual-machine entry routes therefore retain their controller receiver after
persistence instead of relying on legacy string action dispatch. The saved
service remains completion data through link persistence, while route selection
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.421
```
...
Server `v1.6.421` also keeps the embedded-database startup context explicit, so
