...
`v1.6.425` combines Orchestration Engine `0.183.296`, Node Agent `0.13.27`,
...
Server `v1.6.425` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.425
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.425
...
Server `v1.6.425` also keeps the embedded-database startup context explicit, so
