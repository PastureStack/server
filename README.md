...
`v1.6.362` combines Orchestration Engine `0.183.281`, Node Agent `0.13.22`,
...
replacing the retired Bootstrap 3.4.1 stylesheet and Glyphicons with API Explorer `1.1.17`,
...
Server `v1.6.362` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.70` retains the deterministic loading-overlay lifecycle and
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.362
```
