...
This is a compatibility-focused modernization project. Existing Ubuntu 26.04, Java 25, MariaDB, modern Docker, non-root runtime, artifact-integrity, authentication, WebSocket, backup/restore, and test work is retained. Server `v1.6.335` includes the reviewed Ember 6.12 LTS Web Console `1.6.56-pasturestack.46`, readable WCAG AA Catalog documentation code blocks in every compiled theme, reactive Catalog upgrade version choices mapped from the exact API links, native rendering for primitive Catalog enum questions, required-answer validation that accepts valid boolean `false` and numeric `0` values, and localized question fallback across immutable revisions of the same template. It also rejects delayed version responses that no longer match the operator's selected option. The release retains the complete classic-template compatibility bridge, recursive-input guard, writable OpenID Connect provider configuration, broker-aware terminal recovery without a failed HTTP request for an expired browser session, an 11-pixel unobtrusive console resize handle, single-owner project WebSocket reconnect fix, retained installed-Catalog version routes, and API Explorer `1.1.15`, in addition to the stable account-to-login-identity links, transactional provider switching, explicit permission reassignment, administrator-controlled MFA policy, account-level verification throttling, short-lived step-up confirmation, trusted federated-MFA claims, MFA-protected local administrator recovery, TOTP, WebAuthn passkeys, recovery codes, verified email account recovery, and all 13 reviewed production locale assets retained from `v1.6.334`. SMTP delivery is configured once per installation by a system administrator; individual accounts store only their own verified recovery destination address. Passing the documented compatibility gates does not by itself make a deployment production-ready.
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.335
```
