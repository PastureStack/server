...
`v1.6.371` combines Orchestration Engine `0.183.286`, Node Agent `0.13.22`,
...
Server `v1.6.371` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.74` restores the hidden-state compatibility contract required by
the Bootstrap 5 migration, so inactive full-screen overlays no longer intercept
navigation or filter controls. It retains the audit-log filter builder, existing
audit table, Bootstrap runtime boundary, and deterministic loading-overlay lifecycle and
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.371
```
