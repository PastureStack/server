...
`v1.6.413` combines Orchestration Engine `0.183.295`, Node Agent `0.13.27`,
...
Server `v1.6.413` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.103` preserves the Server `v1.6.358` authenticated visual and
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.413
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.413
...
Server `v1.6.413` also keeps the embedded-database startup context explicit, so
