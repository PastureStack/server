...
`v1.6.370` combines Orchestration Engine `0.183.286`, Node Agent `0.13.22`,
...
Server `v1.6.370` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.73` restores Bootstrap 5 navigation collapse and dropdown
execution at application boot while retaining the audit-log filter builder and
existing audit table. It also retains the deterministic loading-overlay lifecycle and
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.370
```
