...
`v1.6.415` combines Orchestration Engine `0.183.295`, Node Agent `0.13.27`,
...
Server `v1.6.415` registers the complete live volume-preflight schema model and preserves the project-scoped authorization required by driver-aware volume configuration, accessible
...
Web Console `1.6.105` preserves the Server `v1.6.358` authenticated visual and
...
submissions. The same guarded completion applies to virtual-machine, load
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.415
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.415
...
Server `v1.6.415` also keeps the embedded-database startup context explicit, so
