...
`v1.6.439` combines Orchestration Engine `0.183.298`, Node Agent `0.13.27`,
...
`v0.3.11`). It retains prior immutable template revisions and adds reviewed
...
```sh
docker run -d --name pasturestack-server --restart unless-stopped -p 8080:8080 ghcr.io/pasturestack/server:v1.6.439
```
...
services:
  pasturestack-server:
    image: ghcr.io/pasturestack/server:v1.6.439
