...
def assert_any_command_running(docker_containers, container_name, candidates):
    processes = docker_containers._get_process_commands(container_name)
    assert any(candidate in process
               for process in processes
               for candidate in candidates)


def assert_no_command_running(docker_containers, container_name, candidates):
    processes = docker_containers._get_process_commands(container_name)
    assert not any(candidate in process
                   for process in processes
                   for candidate in candidates)
...
def test_local_cattle_db_has_mysql_process(docker_containers,
                                           mysql_command_candidates):
    assert_any_command_running(
        docker_containers, 'server_localmysqlcattle_1',
        mysql_command_candidates)


def test_link_cattle_no_mysql_process(docker_containers,
                                      mysql_command_candidates):
    assert_no_command_running(
        docker_containers, 'server_mysqllinkcattle_1',
        mysql_command_candidates)


def test_manual_cattle_no_mysql_process(docker_containers,
                                        mysql_command_candidates):
    assert_no_command_running(
        docker_containers, 'server_mysqlmanualcattle_1',
        mysql_command_candidates)


def test_h2_cattle_no_mysql_process(docker_containers,
                                    mysql_command_candidates):
    assert_no_command_running(
        docker_containers, 'server_h2dbcattle_1',
        mysql_command_candidates)
