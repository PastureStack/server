...
require_marker '9b5115c068d2e6dbfa1cc8ffebb01eaf0e90f171' CATALOG_PIN_MISSING
require_marker "pinned_commit='\${APPROVED_CATALOG_COMMIT}'" CATALOG_PINNED_COMMIT_MIGRATION_MISSING
require_marker 'Keep commit at the last successfully indexed revision' CATALOG_STALE_INDEX_GUARD_MISSING
require_marker "'\${APPROVED_CATALOG_BRANCH}'," CATALOG_INSERT_BRANCH_MISSING
require_marker '                 NULL,' CATALOG_INSERT_MUST_START_UNINDEXED
...
catalog_update_block=$(
    sed -n '/^[[:space:]]*UPDATE catalog$/,/^[[:space:]]*WHERE \${catalog_predicate};$/p' \
        "$script"
)
if grep -Eq "^[[:space:]]*commit='\$\{APPROVED_CATALOG_COMMIT\}'" \
    <<<"$catalog_update_block"; then
    echo CATALOG_UPDATE_PREADVANCES_INDEXED_COMMIT >&2
    exit 1
fi
