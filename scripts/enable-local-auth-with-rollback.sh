...
backup_root=${RC16_BACKUP_ROOT:-}
if [ -z "$backup_root" ]; then
  user_state_root=${XDG_STATE_HOME:-${HOME:?HOME is required when RC16_BACKUP_ROOT and XDG_STATE_HOME are unset}/.local/state}
  backup_root="${user_state_root%/}/pasturestack/backups"
fi
...
WHERE s1.name IN (
  ...
  'api.auth.provider.name.configured',
  'api.auth.external.provider.configured',
  'api.auth.external.provider.no.identity.lookup',
...
UPDATE setting SET value='' WHERE name='api.auth.provider.name.configured';
INSERT INTO setting (name, value)
SELECT 'api.auth.provider.name.configured', ''
WHERE NOT EXISTS (SELECT 1 FROM setting WHERE name='api.auth.provider.name.configured');
UPDATE setting SET value='false' WHERE name='api.auth.external.provider.configured';
INSERT INTO setting (name, value)
SELECT 'api.auth.external.provider.configured', 'false'
WHERE NOT EXISTS (SELECT 1 FROM setting WHERE name='api.auth.external.provider.configured');
UPDATE setting SET value='false' WHERE name='api.auth.external.provider.no.identity.lookup';
INSERT INTO setting (name, value)
SELECT 'api.auth.external.provider.no.identity.lookup', 'false'
WHERE NOT EXISTS (SELECT 1 FROM setting WHERE name='api.auth.external.provider.no.identity.lookup');
UPDATE setting SET value='' WHERE name='api.auth.allowed.identities';
INSERT INTO setting (name, value)
SELECT 'api.auth.allowed.identities', ''
WHERE NOT EXISTS (SELECT 1 FROM setting WHERE name='api.auth.allowed.identities');
UPDATE setting SET value='unrestricted' WHERE name='api.auth.access.mode';
INSERT INTO setting (name, value)
SELECT 'api.auth.access.mode', 'unrestricted'
WHERE NOT EXISTS (SELECT 1 FROM setting WHERE name='api.auth.access.mode');
