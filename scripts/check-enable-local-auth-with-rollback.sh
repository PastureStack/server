...
require_marker scripts/enable-local-auth-with-rollback.sh 'user_state_root=${XDG_STATE_HOME:-${HOME:?HOME is required when RC16_BACKUP_ROOT and XDG_STATE_HOME are unset}/.local/state}' LOCAL_AUTH_ROLLBACK_PORTABLE_STATE_ROOT_MISSING
require_marker scripts/enable-local-auth-with-rollback.sh 'backup_root="${user_state_root%/}/pasturestack/backups"' LOCAL_AUTH_ROLLBACK_PORTABLE_BACKUP_ROOT_MISSING
require_marker scripts/enable-local-auth-with-rollback.sh "UPDATE setting SET value='' WHERE name='api.auth.provider.name.configured';" LOCAL_AUTH_ROLLBACK_REMEMBERED_PROVIDER_NOT_CLEARED
require_marker scripts/enable-local-auth-with-rollback.sh "UPDATE setting SET value='false' WHERE name='api.auth.external.provider.configured';" LOCAL_AUTH_ROLLBACK_EXTERNAL_PROVIDER_FLAG_NOT_CLEARED
require_marker scripts/enable-local-auth-with-rollback.sh "UPDATE setting SET value='false' WHERE name='api.auth.external.provider.no.identity.lookup';" LOCAL_AUTH_ROLLBACK_EXTERNAL_LOOKUP_FLAG_NOT_CLEARED
require_marker scripts/enable-local-auth-with-rollback.sh "UPDATE setting SET value='' WHERE name='api.auth.allowed.identities';" LOCAL_AUTH_ROLLBACK_EXTERNAL_IDENTITIES_NOT_CLEARED
require_marker scripts/enable-local-auth-with-rollback.sh "UPDATE setting SET value='unrestricted' WHERE name='api.auth.access.mode';" LOCAL_AUTH_ROLLBACK_EXTERNAL_ACCESS_MODE_NOT_RESET
...
reject_marker scripts/enable-local-auth-with-rollback.sh '/home/' LOCAL_AUTH_ROLLBACK_MACHINE_SPECIFIC_HOME_PATH_PRESENT
...
printf 'LOCAL_AUTH_ROLLBACK_OK strict_shell=1 curl_bounded=1 ping_sample=1 api_samples=1 portable_backup_root=1 external_provider_state_reset=1\n'
