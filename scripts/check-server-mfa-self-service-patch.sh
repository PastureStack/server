...
release_doc=docs/releases/server-1.6.321.md
...
require_marker "$dockerfile" \
    'org.opencontainers.image.version="v1.6.321"' \
...
require_marker "$dockerfile" \
    'ENV PASTURESTACK_WEB_CONSOLE_PACKAGE=1.6.56-pasturestack.33' \
...
require_marker "$dockerfile" \
    'test "$(basename "${old_web_root}")" = "${engine_hash}"' \
    SERVER_MFA_SELF_SERVICE_SAME_ENGINE_ROOT_VERIFICATION_MISSING
require_marker "$dockerfile" \
    'ln -s "${engine_hash}" /usr/share/cattle/war' \
    SERVER_MFA_SELF_SERVICE_HASH_ROOT_LINK_MISSING
require_marker "$dockerfile" \
    'mkdir -p /usr/share/cattle/war/assets;' \
    SERVER_MFA_SELF_SERVICE_ASSET_DIRECTORY_BOOTSTRAP_MISSING
require_marker "$dockerfile" \
    'ui-dark.rtl.css' \
    SERVER_MFA_SELF_SERVICE_THEME_ASSET_GATE_MISSING
require_marker "$dockerfile" \
...
printf 'SERVER_MFA_SELF_SERVICE_PATCH_OK release=v1.6.321 engine=0.183.272 authentication_service=0.2.5 web_console=1.6.56-pasturestack.33 base=v1.6.319 locales=13 runtime_digest_coordinates=0\n'
