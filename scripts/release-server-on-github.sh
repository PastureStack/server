...
catalog_commit=18cc041a293a2760fab23e31cc832be1d2084a27
...
wait_for_catalog()
{
  ...
  local container_schedule_response="$work_root/catalog-container-schedule-${container}.json"
  local system_image_preloader_response="$work_root/catalog-system-image-preloader-${container}.json"
  local route53_dns_sync_response="$work_root/catalog-route53-dns-sync-${container}.json"
  local amazon_ebs_storage_response="$work_root/catalog-amazon-ebs-storage-${container}.json"
  local amazon_efs_storage_response="$work_root/catalog-amazon-efs-storage-${container}.json"
  local network_diagnostics_response="$work_root/catalog-network-diagnostics-${container}.json"
  for _ in $(seq 1 180); do
    if curl -fsS --connect-timeout 2 --max-time 10 \
      "http://127.0.0.1:${port}/v1-catalog/templates?limit=-1" \
      -o "$response" 2>/dev/null &&
      jq -e '(.data // []) | length == 16' "$response" >/dev/null &&
      ...
      ' "$ipsec_response" >/dev/null &&
      curl --globoff -fsS --connect-timeout 2 --max-time 10 \
        "http://127.0.0.1:${port}/v1-catalog/templates/pasturestack:infra*container-schedule:1" \
        -o "$container_schedule_response" 2>/dev/null &&
      jq -e '
        .id == "pasturestack:infra*container-schedule:1" and
        (.files["docker-compose.yml.tpl"] // "") as $compose |
        ($compose | contains(
          "ghcr.io/pasturestack/container-cron:v0.6.0"
        )) and
        ($compose | contains("/var/run/docker.sock:/var/run/docker.sock")) and
        (($compose | contains("@sha256:")) | not)
      ' "$container_schedule_response" >/dev/null &&
      curl --globoff -fsS --connect-timeout 2 --max-time 10 \
        "http://127.0.0.1:${port}/v1-catalog/templates/pasturestack:infra*system-image-preloader:1" \
        -o "$system_image_preloader_response" 2>/dev/null &&
      jq -e '
        .id == "pasturestack:infra*system-image-preloader:1" and
        (.files["docker-compose.yml.tpl"] // "") as $compose |
        ($compose | contains(
          "ghcr.io/pasturestack/system-image-preloader:v0.3.0"
        )) and
        ($compose | contains("/var/run/docker.sock:/var/run/docker.sock")) and
        ($compose | contains("io.rancher.container.start_once")) and
        ($compose | contains("io.rancher.scheduler.global")) and
        (($compose | contains("@sha256:")) | not)
      ' "$system_image_preloader_response" >/dev/null &&
      curl --globoff -fsS --connect-timeout 2 --max-time 10 \
        "http://127.0.0.1:${port}/v1-catalog/templates/pasturestack:infra*route53-dns-sync:1" \
        -o "$route53_dns_sync_response" 2>/dev/null &&
      jq -e '
        .id == "pasturestack:infra*route53-dns-sync:1" and
        (.files["docker-compose.yml.tpl"] // "") as $compose |
        ($compose | contains(
          "ghcr.io/pasturestack/external-dns-sync:v0.8.0"
        )) and
        ($compose | contains("- -provider=route53")) and
        (($compose | contains("@sha256:")) | not) and
        (($compose | contains("privileged:")) | not) and
        (($compose | contains("network_mode: host")) | not) and
        (($compose | contains("/var/run/docker.sock")) | not)
      ' "$route53_dns_sync_response" >/dev/null &&
      curl --globoff -fsS --connect-timeout 2 --max-time 10 \
        "http://127.0.0.1:${port}/v1-catalog/templates/pasturestack:infra*amazon-ebs-storage:1" \
        -o "$amazon_ebs_storage_response" 2>/dev/null &&
      jq -e '
        .id == "pasturestack:infra*amazon-ebs-storage:1" and
        (.files["docker-compose.yml"] // "") as $compose |
        ($compose | contains(
          "ghcr.io/pasturestack/ebs-storage-driver:v0.10.0"
        )) and
        ($compose | contains("ALLOW_CLOUD_PROVISIONING")) and
        ($compose | contains(
          "/var/lib/rancher/volumes:/var/lib/pasturestack/volumes:shared"
        )) and
        (($compose | contains("@sha256:")) | not)
      ' "$amazon_ebs_storage_response" >/dev/null &&
      curl --globoff -fsS --connect-timeout 2 --max-time 10 \
        "http://127.0.0.1:${port}/v1-catalog/templates/pasturestack:infra*amazon-efs-storage:1" \
        -o "$amazon_efs_storage_response" 2>/dev/null &&
      jq -e '
        .id == "pasturestack:infra*amazon-efs-storage:1" and
        (.files["docker-compose.yml"] // "") as $compose |
        ($compose | contains(
          "ghcr.io/pasturestack/efs-storage-driver:v0.10.0"
        )) and
        ($compose | contains("ALLOW_CLOUD_PROVISIONING")) and
        ($compose | contains("EFS_SECURITY_GROUP_ID")) and
        (($compose | contains("0.0.0.0/0")) | not) and
        (($compose | contains("@sha256:")) | not)
      ' "$amazon_efs_storage_response" >/dev/null &&
      curl --globoff -fsS --connect-timeout 2 --max-time 10 \
        "http://127.0.0.1:${port}/v1-catalog/templates/pasturestack:infra*network-diagnostics:1" \
        -o "$network_diagnostics_response" 2>/dev/null &&
      jq -e '
        .id == "pasturestack:infra*network-diagnostics:1" and
        (.files["docker-compose.yml.tpl"] // "") as $compose |
        ($compose | contains(
        ...
        ($compose | contains("io.rancher.scheduler.global")) and
        (($compose | contains("@sha256:")) | not) and
        (($compose | contains("privileged: true")) | not) and
        (($compose | contains("network_mode: host")) | not) and
        (($compose | contains("/var/run/docker.sock")) | not)
      ' "$network_diagnostics_response" >/dev/null; then
      return 0
    fi
    sleep 1
  done
  ...
  test -f "$container_schedule_response" &&
    cat "$container_schedule_response" >&2
  test -f "$system_image_preloader_response" &&
    cat "$system_image_preloader_response" >&2
  test -f "$route53_dns_sync_response" &&
    cat "$route53_dns_sync_response" >&2
  test -f "$amazon_ebs_storage_response" &&
    cat "$amazon_ebs_storage_response" >&2
  test -f "$amazon_efs_storage_response" &&
    cat "$amazon_efs_storage_response" >&2
  test -f "$network_diagnostics_response" &&
    cat "$network_diagnostics_response" >&2
  docker logs --tail 300 "$container" >&2 || true
  return 1
}
