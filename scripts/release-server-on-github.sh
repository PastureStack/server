...
catalog_commit=9b5115c068d2e6dbfa1cc8ffebb01eaf0e90f171
...
wait_for_catalog()
{
  ...
  for _ in $(seq 1 180); do
    if curl -fsS --connect-timeout 2 --max-time 10 \
      ...
      jq -e '(.data // []) | length == 13' "$response" >/dev/null &&
      ...
      ' "$route53_dns_sync_response" >/dev/null; then
      return 0
    fi
    sleep 1
  done
