...
catalog_commit=faa122271462c52cc6c6c7a461d9e649e232e9fa
...
wait_for_catalog()
{
  ...
  for _ in $(seq 1 180); do
    if curl -fsS --connect-timeout 2 --max-time 10 \
      ...
      jq -e '(.data // []) | length == 15' "$response" >/dev/null &&
      ...
      jq -e '
        ...
        (($compose | contains("@sha256:")) | not)
      ' "$amazon_efs_storage_response" >/dev/null; then
      return 0
    fi
    sleep 1
  done
