...
catalog_commit=3cfb447d7564cf9bada4bac2e15ce3dd6b221615
...
wait_for_catalog()
{
  ...
  for _ in $(seq 1 180); do
    if curl -fsS --connect-timeout 2 --max-time 10 \
      ...
      jq -e '(.data // []) | length == 11' "$response" >/dev/null &&
      ...
      jq -e '
        ...
        (($compose | contains("@sha256:")) | not)
      ' "$system_image_preloader_response" >/dev/null; then
      return 0
    fi
    sleep 1
  done
