...
catalog_commit=8e4caa289895855df9706ebf625f401da4ca2e5b
...
wait_for_catalog()
{
  ...
  for _ in $(seq 1 180); do
    if curl -fsS --connect-timeout 2 --max-time 10 \
      ...
      jq -e '(.data // []) | length == 10' "$response" >/dev/null &&
      ...
      jq -e '
        ...
        (($compose | contains("@sha256:")) | not)
      ' "$container_schedule_response" >/dev/null; then
      return 0
    fi
    sleep 1
  done
