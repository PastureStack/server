...
release_notes=docs/releases/server-1.6.404.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.404"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.404' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.290' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.290.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=6d1af6126777dfeccf4722f96a049abff1621f217a4a01f9bee6824dedb3fb21' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=8cec53e69ea30157273dbd5089edb4a79fad7998' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.290'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.290' \
...
require_marker "$dockerfile" \
    'grep -Fx '\''Implementation-Version: 0.183.290'\'' >/dev/null' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.404"
