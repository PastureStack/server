...
current_release_notes=docs/releases/server-1.6.456.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.456"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.456' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.315' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.315.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=8d92bcf24ff26340f7e92013408d46c81377965d2f9ef773936ca0118895ef8d' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=9faa58a7bf565999a9d219200867996a85eb8a2d' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.315'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.315' \
...
for current_release_marker in \
    ...
    '`9faa58a7bf565999a9d219200867996a85eb8a2d`' \
    ...
    '`token` account on upgraded installations' \
    'match exactly' \
    'oidc_user' \
    'oidc_group' \
    'six-account'; do
...
for current_readme_marker in \
    '`v1.6.456` combines Orchestration Engine `0.183.315`' \
    '`token` account on upgraded installations' \
...
for current_compatibility_marker in \
    ...
    'identity ownership across repeated OIDC logins' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.456"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.456 >/dev/null
