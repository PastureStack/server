...
release_notes=docs/releases/server-1.6.434.md
...
for path in "$dockerfile" "$release_dockerfile" "$build_script" "$publish_workflow" "$cattle_script" \
    ...
    "$release_notes" "$host_api_repair" "$host_api_check"; do
    test -f "$path"
done
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.434"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.434' \
...
for release_engine_marker in \
    ...
    "grep -Fx 'Implementation-Version: 0.183.298'" \
...
require_marker "$release_dockerfile" \
    'ENV PASTURESTACK_CATALOG_COMMIT=b0a36559e91a539c87fd38bae663c4b41a112fcf' \
    SERVER_CATALOG_VERSION_LABEL_COMMIT_MISSING
require_marker "$release_dockerfile" \
    '"pinnedCommit":"b0a36559e91a539c87fd38bae663c4b41a112fcf"' \
    SERVER_CATALOG_VERSION_LABEL_URL_MISSING
require_marker "$build_script" \
    'PASTURESTACK_CATALOG_COMMIT=b0a36559e91a539c87fd38bae663c4b41a112fcf' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.434"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.434 >/dev/null
