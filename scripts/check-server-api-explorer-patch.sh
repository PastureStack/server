...
current_release_notes=docs/releases/server-1.6.453.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.453"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.453' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.312' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.312.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=d4f5003e7e73af6bfc228dc598ade0c49dea22652f30f1d2527cd98de75623e6' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=8462f1fbe1b034caea1cd10304ec3db2078dbb80' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.312'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.312' \
...
for current_release_marker in \
    ...
    '`8462f1fbe1b034caea1cd10304ec3db2078dbb80`' \
...
for current_readme_marker in \
    '`v1.6.453` combines Orchestration Engine `0.183.312`' \
...
for current_compatibility_marker in \
    'Orchestration Engine `v0.183.312`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.453"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.453 >/dev/null
