...
current_release_notes=docs/releases/server-1.6.469.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.469"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.469' \
...
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.132' \
    SERVER_INCREMENTAL_WEB_CONSOLE_VERSION_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=ffb000508cb08a149e34633121aec25f4dab022967c1f4cdf917260061f0dace' \
    SERVER_INCREMENTAL_WEB_CONSOLE_HASH_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_COMMIT=4c31bf8b11c6924c2415f7b178afafc2e18b696e' \
...
for current_readme_marker in \
    '`v1.6.469` combines Orchestration Engine `0.183.322`' \
    '[v1.6.469 release notes](docs/releases/server-1.6.469.md)' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.469"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.469 >/dev/null
