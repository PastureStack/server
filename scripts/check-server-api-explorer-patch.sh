...
release_notes=docs/releases/server-1.6.437.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.437"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.437' \
...
require_marker "$release_dockerfile" \
    'ENV PASTURESTACK_CATALOG_COMMIT=9d487b1cf3681e6718058b456d218b7742d7b96b' \
    SERVER_CATALOG_VERSION_LABEL_COMMIT_MISSING
require_marker "$release_dockerfile" \
    '"pinnedCommit":"9d487b1cf3681e6718058b456d218b7742d7b96b"' \
    SERVER_CATALOG_VERSION_LABEL_URL_MISSING
require_marker "$build_script" \
    'PASTURESTACK_CATALOG_COMMIT=9d487b1cf3681e6718058b456d218b7742d7b96b' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.437"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.437 >/dev/null
