...
current_release_notes=docs/releases/server-1.6.463.md
...
for path in "$dockerfile" "$release_dockerfile" "$core_dockerfile" "$build_script" "$publish_workflow" "$cattle_script" \
    ...
    "$release_notes" "$previous_release_notes" "$current_release_notes" "$host_api_repair" "$host_api_check" \
    "$mfa_policy_smoke"; do
    test -f "$path"
done
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.463"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.463' \
...
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.126' \
    SERVER_INCREMENTAL_WEB_CONSOLE_VERSION_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=a278904a10ce757510ed516507bd3926b02bab42a52a27bd153ff5e94e2998aa' \
...
require_marker "$build_script" \
    'image=${IMAGE:-pasturestack-validation/server:v1.6.463}' \
    SERVER_INCREMENTAL_RELEASE_BUILD_VERSION_MISSING
require_marker "$build_script" \
    'CATTLE_RANCHER_SERVER_VERSION=v1.6.463' \
...
require_marker "$build_script" \
    'test "$(cat "${web_root}/VERSION.txt")" = "1.6.126"' \
...
for current_release_marker in \
    ...
    'unchanged Web Console 1.6.126' \
    '`268469153c299987bd42e288dd68bb62af91002e`' \
    '`cattle.jar`' \
    '`8483db0b4f2fe71ce527ba97bfb3caea14096ec356124ecef9aa1e7853c8553e`' \
    '`cattle-cyclonedx.json`' \
    '`9869f1d6bd9ca5a193a89bc5d68d5ccae79b26291c320766df7332c078c7ae7e`' \
    'X-API-Project-Id: A' \
    '`?projectId=B`' \
    'Rollback uses the preserved v1.6.462 image'; do
...
for current_readme_marker in \
    '`v1.6.463` combines Orchestration Engine `0.183.320`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.463"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.463 >/dev/null
