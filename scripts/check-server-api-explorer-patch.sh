...
release_notes=docs/releases/server-1.6.436.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.436"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.436' \
...
require_marker "$release_dockerfile" \
    'ENV PASTURESTACK_CATALOG_COMMIT=4adf274d699a2f4e729f64a951a5846523e19acc' \
    SERVER_CATALOG_VERSION_LABEL_COMMIT_MISSING
require_marker "$release_dockerfile" \
    '"pinnedCommit":"4adf274d699a2f4e729f64a951a5846523e19acc"' \
    SERVER_CATALOG_VERSION_LABEL_URL_MISSING
require_marker "$build_script" \
    'PASTURESTACK_CATALOG_COMMIT=4adf274d699a2f4e729f64a951a5846523e19acc' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.436"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.436 >/dev/null
