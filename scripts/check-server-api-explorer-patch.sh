...
current_release_notes=docs/releases/server-1.6.459.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.459"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.459' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.317' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.317.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=24d143a2ba674ad0a62bd22c610354ba4e89ab9e84bf5e57eb5d894142c30d01' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=aeca90a9699089b68691dd863b7fff2ad4744a64' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.317'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.317' \
...
for current_release_marker in \
    ...
    '`aeca90a9699089b68691dd863b7fff2ad4744a64`' \
    ...
    '`adminProject`' \
    '`noaccess`' \
    'six independent OpenID Connect accounts'; do
...
for current_readme_marker in \
    '`v1.6.459` combines Orchestration Engine `0.183.317`' \
...
for current_compatibility_marker in \
    'Orchestration Engine `v0.183.317`' \
    'single `adminProject` Default' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.459"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.459 >/dev/null
