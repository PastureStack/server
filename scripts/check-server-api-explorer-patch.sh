...
current_release_notes=docs/releases/server-1.6.452.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.452"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.452' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.311' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.311.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=24d710e50c44dbf4bbea65d0ce02091d7554643560cc5f28b84e21df08a5be60' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=b4507f9e969afd685e0df87902211690e7350075' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.311'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.311' \
...
for current_release_marker in \
    ...
    '`b4507f9e969afd685e0df87902211690e7350075`' \
...
for current_readme_marker in \
    '`v1.6.452` combines Orchestration Engine `0.183.311`' \
...
for current_compatibility_marker in \
    'Orchestration Engine `v0.183.311`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.452"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.452 >/dev/null
