...
current_release_notes=docs/releases/server-1.6.462.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.462"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.462' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.319' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.319.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=74ac55939399873eeb9ab0813ca193e373ccf2bdde3e3d884d5491b4d0b7608e' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=222552c4b1fad095a5b55756cdca8e02b088ba04' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.319'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.319' \
...
for current_readme_marker in \
    '`v1.6.462` combines Orchestration Engine `0.183.319`' \
...
for current_compatibility_marker in \
    'Orchestration Engine `v0.183.319`' \
    'single `adminProject` Default' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.462"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.462 >/dev/null
