...
release_notes=docs/releases/server-1.6.393.md
...
require_marker "$dockerfile" \
    'org.opencontainers.image.version="v1.6.393"' \
    SERVER_API_EXPLORER_PATCH_VERSION_MISSING
require_marker "$dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.393' \
...
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.95' \
    SERVER_WEB_CONSOLE_RELEASE_TAG_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT=web-console-1.6.95.tar.gz' \
    SERVER_WEB_CONSOLE_RELEASE_ARTIFACT_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=49d04ab9ee5678a206e7f52057b22d3006bd0a0bddfc938a1a18900845c60bf8' \
    SERVER_WEB_CONSOLE_RELEASE_HASH_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_COMMIT=de0b9fd4664bdce3f4a487f73f8ec51c59b10c3c' \
...
require_marker "$build_script" \
    'test "$(cat "${web_root}/VERSION.txt")" = "1.6.95"' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.393"
