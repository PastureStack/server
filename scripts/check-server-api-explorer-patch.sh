...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.439"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.439' \
...
require_marker "$release_dockerfile" \
    'ENV PASTURESTACK_CATALOG_COMMIT=b083630a34d1a9a1cf48006389187bcf90a6b4da' \
    SERVER_CATALOG_VERSION_LABEL_COMMIT_MISSING
require_marker "$release_dockerfile" \
    '"pinnedCommit":"b083630a34d1a9a1cf48006389187bcf90a6b4da"' \
    SERVER_CATALOG_VERSION_LABEL_URL_MISSING
require_marker "$build_script" \
    'PASTURESTACK_CATALOG_COMMIT=b083630a34d1a9a1cf48006389187bcf90a6b4da' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.439"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.439 >/dev/null
