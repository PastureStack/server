...
current_release_notes=docs/releases/server-1.6.455.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.455"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.455' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.314' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.314.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=e685d6f65b2abd45e36fa28fd3db4223a03a1a1a6e316910e19eb0f84c8bd297' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=68b194b96dadee0f60a4b94e027ea1caac8be018' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.314'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.314' \
...
for current_release_marker in \
    ...
    '`68b194b96dadee0f60a4b94e027ea1caac8be018`' \
...
for current_readme_marker in \
    '`v1.6.455` combines Orchestration Engine `0.183.314`' \
...
for current_compatibility_marker in \
    'Orchestration Engine `v0.183.314`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.455"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.455 >/dev/null
