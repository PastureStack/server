...
current_release_notes=docs/releases/server-1.6.457.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.457"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.457' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.316' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.316.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=5439563f2e2c1be5ac0e00965080a06df92f35680c816d4e9219a1a1f903d161' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=023a95f542f037617fb4de5a528dd02c73b28d7c' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.316'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.316' \
...
for current_release_marker in \
    ...
    '`023a95f542f037617fb4de5a528dd02c73b28d7c`' \
...
for current_readme_marker in \
    '`v1.6.457` combines Orchestration Engine `0.183.316`' \
    'local recovery administrator' \
...
for current_compatibility_marker in \
    'Orchestration Engine `v0.183.316`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.457"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.457 >/dev/null
