...
current_release_notes=docs/releases/server-1.6.458.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.458"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.458' \
...
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.123' \
    SERVER_INCREMENTAL_WEB_CONSOLE_VERSION_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=be9aa07ac82548c6c538850fae1a08482c06a3fafb78efe4d0fe74808d411a4a' \
...
require_marker "$build_script" \
    'test "$(cat "${web_root}/VERSION.txt")" = "1.6.123"' \
...
for current_release_marker in \
    '# Server v1.6.458' \
    '`78705efff4b6b850d4ae9f6b06e90e0a62165311`' \
...
for current_readme_marker in \
    '`v1.6.458` combines Orchestration Engine `0.183.317`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.458"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.458 >/dev/null
