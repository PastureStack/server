...
release_notes=docs/releases/server-1.6.372.md
...
require_marker "$dockerfile" \
    'org.opencontainers.image.version="v1.6.372"' \
    SERVER_API_EXPLORER_PATCH_VERSION_MISSING
require_marker "$dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.372' \
...
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.75' \
    SERVER_WEB_CONSOLE_RELEASE_TAG_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT=web-console-1.6.75.tar.gz' \
    SERVER_WEB_CONSOLE_RELEASE_ARTIFACT_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=d27442f880bb42a91441019dc9ef7f780035065389f981bcbe07be07db059ecb' \
    SERVER_WEB_CONSOLE_RELEASE_HASH_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_COMMIT=7499f6e49cd5cc3acfd488f89cae95b618d8b114' \
...
require_marker "$build_script" \
    'test "$(cat "${web_root}/VERSION.txt")" = "1.6.75"' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.372"
...
printf 'SERVER_API_EXPLORER_PATCH_OK release=v1.6.372 base=v1.6.364 orchestration=0.183.286 distributed_cache=5.7.3-pasturestack.4 api_explorer=1.1.18 web_console=1.6.75 audit_log_filters=1 dropdown_destination=1 docker_29_range=29.4.1..29.7.2 docker_29_6_2=supported bootstrap=5.3.8 bootstrap_icons=1.13.1 bootstrap_javascript=0 runtime_go=1.27.0 ubuntu_security_refresh=2026-08-26 coreutils_uniq=9.11+d64e35a8 openssl=3.5.8 zlib=1.3.2 diff3=removed source_build_mode=removed runtime_tar=removed ssh_client=removed mount_helpers=removed runtime_digest_coordinates=1 vex=openvex-0.2.0 unresolved=0 legal_assets=complete\n'
