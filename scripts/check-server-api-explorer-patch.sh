...
current_release_notes=docs/releases/server-1.6.454.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.454"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.454' \
...
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.313' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=orchestration-engine-0.183.313.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=0725da73f2fe67d2e3b9788b8e9d11c62cb47d99fd2b94c9da3c0dea05532faa' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=78a73cb4b6c42ba5b10b46abd846a62abbd3fc35' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.313'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.313' \
...
for current_release_marker in \
    ...
    '`78a73cb4b6c42ba5b10b46abd846a62abbd3fc35`' \
...
for current_readme_marker in \
    '`v1.6.454` combines Orchestration Engine `0.183.313`' \
...
for current_compatibility_marker in \
    'Orchestration Engine `v0.183.313`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.454"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.454 >/dev/null
