...
current_release_notes=docs/releases/server-1.6.464.md
...
for path in "$dockerfile" "$release_dockerfile" "$core_dockerfile" "$build_script" "$publish_workflow" "$cattle_script" \
    ...
    "$release_notes" "$previous_release_notes" "$last_release_notes" "$current_release_notes" "$host_api_repair" "$host_api_check" \
    "$mfa_policy_smoke"; do
    test -f "$path"
done
...
if grep -Fq '__WEB_CONSOLE_1_6_127_' "$release_dockerfile" "$build_script"; then
    echo 'SERVER_WEB_CONSOLE_RELEASE_COORDINATES_PENDING' >&2
    exit 1
fi
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.464"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.464' \
...
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.127' \
    SERVER_INCREMENTAL_WEB_CONSOLE_VERSION_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=dcaaad16bbdf38c82af1cecb9cfc0867dcceab76503f632e151da6a17952f852' \
    SERVER_INCREMENTAL_WEB_CONSOLE_HASH_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_COMMIT=0f21118a871a1b72558a2fa11b4f33675fd6ae15' \
    SERVER_INCREMENTAL_WEB_CONSOLE_COMMIT_MISSING
require_marker "$build_script" \
    'web_console_commit=${WEB_CONSOLE_COMMIT:-0f21118a871a1b72558a2fa11b4f33675fd6ae15}' \
...
require_marker "$build_script" \
    'image=${IMAGE:-pasturestack-validation/server:v1.6.464}' \
    SERVER_INCREMENTAL_RELEASE_BUILD_VERSION_MISSING
require_marker "$build_script" \
    'CATTLE_RANCHER_SERVER_VERSION=v1.6.464' \
    SERVER_INCREMENTAL_RELEASE_BUILD_RUNTIME_VERSION_MISSING
for release_engine_marker in \
    'ARG ORCHESTRATION_ENGINE_RELEASE_TAG=v0.183.320' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT=cattle.jar' \
    'ARG ORCHESTRATION_ENGINE_ARTIFACT_SHA256=8483db0b4f2fe71ce527ba97bfb3caea14096ec356124ecef9aa1e7853c8553e' \
    'ARG ORCHESTRATION_ENGINE_COMMIT=268469153c299987bd42e288dd68bb62af91002e' \
    'COPY --from=release_artifacts /out/orchestration-engine.jar /tmp/orchestration-engine.jar' \
    "grep -Fx 'Implementation-Version: 0.183.320'" \
    'ENV CATTLE_CATTLE_VERSION=v0.183.320' \
...
for release_engine_build_marker in \
    'orchestration_engine_release_tag=${ORCHESTRATION_ENGINE_RELEASE_TAG:-v0.183.320}' \
    'orchestration_engine_artifact=${ORCHESTRATION_ENGINE_ARTIFACT:-cattle.jar}' \
    'orchestration_engine_artifact_sha256=${ORCHESTRATION_ENGINE_ARTIFACT_SHA256:-8483db0b4f2fe71ce527ba97bfb3caea14096ec356124ecef9aa1e7853c8553e}' \
    'orchestration_engine_commit=${ORCHESTRATION_ENGINE_COMMIT:-268469153c299987bd42e288dd68bb62af91002e}' \
    'CATTLE_CATTLE_VERSION=v0.183.320' \
    'cattle-resources-0.183.320.jar'; do
...
require_marker "$build_script" \
    'test "$(cat "${web_root}/VERSION.txt")" = "1.6.127"' \
...
for current_release_marker in \
    ...
    'Web Console 1.6.127' \
    '`0f21118a871a1b72558a2fa11b4f33675fd6ae15`' \
    '`580cb3bc472b9e8105787d76ca20b0189ace84bf`' \
    '`dcaaad16bbdf38c82af1cecb9cfc0867dcceab76503f632e151da6a17952f852`' \
    'environment member' \
    '403' \
    '404' \
    'Rollback uses the preserved v1.6.463 image'; do
...
for current_readme_marker in \
    '`v1.6.464` combines Orchestration Engine `0.183.320`' \
    '[v1.6.464 release notes](docs/releases/server-1.6.464.md)' \
    'single `adminProject` Default' \
    'effective-schema' \
    'local recovery administrator' \
    'Authentication Service `0.4.42`' \
    'Web Console `1.6.127`' \
    'zero-environment' \
    'The release matrix compares complete v1 and' \
    'v2-beta project and stack sets' \
    'removes the remaining timing-sensitive OpenID Connect token' \
...
for current_compatibility_marker in \
    'The current `v1.6.464` assembly consumes Orchestration Engine `v0.183.320`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.464"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.464 >/dev/null
