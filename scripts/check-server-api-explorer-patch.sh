...
current_release_notes=docs/releases/server-1.6.468.md
...
require_marker "$release_dockerfile" \
    'org.opencontainers.image.version="v1.6.468"' \
...
require_marker "$release_dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.468' \
...
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.131' \
    SERVER_INCREMENTAL_WEB_CONSOLE_VERSION_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=c596d198c3f0789391f35f95b481586ffb34443f70429920b4a46a25a762a0b8' \
    SERVER_INCREMENTAL_WEB_CONSOLE_HASH_MISSING
require_marker "$release_dockerfile" \
    'ARG WEB_CONSOLE_COMMIT=c90aeb01d2d6eafd3f791793c1bc96d27a44d138' \
    SERVER_INCREMENTAL_WEB_CONSOLE_COMMIT_MISSING
require_marker "$build_script" \
    'web_console_commit=${WEB_CONSOLE_COMMIT:-c90aeb01d2d6eafd3f791793c1bc96d27a44d138}' \
...
require_marker "$build_script" \
    'image=${IMAGE:-pasturestack-validation/server:v1.6.468}' \
    SERVER_INCREMENTAL_RELEASE_BUILD_VERSION_MISSING
require_marker "$build_script" \
    'CATTLE_RANCHER_SERVER_VERSION=v1.6.468' \
...
require_marker "$build_script" \
    'test "$(cat "${web_root}/VERSION.txt")" = "1.6.131"' \
...
for current_readme_marker in \
    '`v1.6.468` combines Orchestration Engine `0.183.322`' \
...
for current_compatibility_marker in \
    'The current `v1.6.468` assembly consumes Orchestration Engine `v0.183.322`' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.468"
...
bash "$vendor_pending_validator" "$runtime_vendor_pending" \
    "$vendor_pending_fixture" v1.6.468 >/dev/null
