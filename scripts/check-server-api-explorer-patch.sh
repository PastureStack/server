...
require_marker "$dockerfile" \
    'org.opencontainers.image.version="v1.6.375"' \
    SERVER_API_EXPLORER_PATCH_VERSION_MISSING
require_marker "$dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.375' \
...
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_RELEASE_TAG=1.6.78' \
    SERVER_WEB_CONSOLE_RELEASE_TAG_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT=web-console-1.6.78.tar.gz' \
    SERVER_WEB_CONSOLE_RELEASE_ARTIFACT_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_ARTIFACT_SHA256=00430933b0f0f1ae259510b6e2f0999972c6d5bfa3de6e80ce968ba6d3c5140f' \
    SERVER_WEB_CONSOLE_RELEASE_HASH_MISSING
require_marker "$dockerfile" \
    'ARG WEB_CONSOLE_COMMIT=9eeb4e6cdf9c66113483b1f5a6cc342e3acbfa9e' \
...
require_marker "$build_script" \
    'test "$(cat "${web_root}/VERSION.txt")" = "1.6.78"' \
...
jq -e '
  ...
  and .["@id"] == "https://github.com/PastureStack/server/security/openvex/v1.6.375"
...
printf 'SERVER_API_EXPLORER_PATCH_OK release=v1.6.375 base=v1.6.364 orchestration=0.183.286 distributed_cache=5.7.3-pasturestack.4 api_explorer=1.1.18 web_console=1.6.78 audit_log_filters=1 dropdown_destination=1 locale_compatibility=1 operator_state=1 login_experience=1 docker_29_range=29.4.1..29.7.2 docker_29_6_2=supported bootstrap=5.3.8 bootstrap_icons=1.13.1 bootstrap_javascript=0 runtime_go=1.27.0 ubuntu_security_refresh=2026-08-26 coreutils_uniq=9.11+d64e35a8 openssl=3.5.8 zlib=1.3.2 diff3=removed source_build_mode=removed runtime_tar=removed ssh_client=removed mount_helpers=removed runtime_digest_coordinates=1 vex=openvex-0.2.0 unresolved=0 legal_assets=complete\n'
