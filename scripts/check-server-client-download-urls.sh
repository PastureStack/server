...
require_marker 'local compose_version="${CATTLE_RANCHER_COMPOSE_VERSION:-v0.14.31}"' SERVER_COMPOSE_DOWNLOAD_VERSION_NOT_CURRENT
...
require_marker 'CATTLE_MASTER source-build mode has been removed' SERVER_SOURCE_BUILD_MODE_REJECTION_MISSING
...
reject_marker 'git clone' SERVER_SOURCE_BUILD_GIT_REMAINS
reject_marker 'apt-get install' SERVER_SOURCE_BUILD_PACKAGE_INSTALL_REMAINS

printf 'failure_count=%s\n' "$failures"
[ "$failures" -eq 0 ]
printf 'SERVER_CLIENT_DOWNLOAD_URLS_OK compose=0.14.31 cli=0.6.14 flat_release_assets=6 legacy_subdirectories=0 source_build_mode=removed\n'
