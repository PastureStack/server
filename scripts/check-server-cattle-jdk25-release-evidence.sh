...
release_file="docs/releases/rc16-cattle-${expected_cattle_version}.md"
...
if [ -z "$expected_cattle_version" ]; then
  ...
  require_marker "cattle-${expected_cattle_version}.jar" SERVER_CATTLE_RELEASE_PRIMARY_ARTIFACT_SHA_MISSING
  require_marker "cattle-iaas-auth-logic-rc16-${expected_cattle_version}.jar" SERVER_CATTLE_RELEASE_AUTH_ARTIFACT_SHA_MISSING
fi
...
printf 'SERVER_CATTLE_JDK25_RELEASE_EVIDENCE_OK cattle=%s release_file=%s full_package=1 bytecode_major=69 packaged_lib_hygiene=1 standalone=1 artifact_sha=1\n' \
  "$expected_cattle_version" "$release_file"
