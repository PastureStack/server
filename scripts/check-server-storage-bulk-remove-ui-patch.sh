...
require_marker "$dockerfile" \
    'ARG BASE_IMAGE=ghcr.io/pasturestack/server:v1.6.310' \
    SERVER_STORAGE_BULK_REMOVE_UI_BASE_NOT_CURRENT
require_marker "$dockerfile" \
    'org.opencontainers.image.version="v1.6.311"' \
    SERVER_STORAGE_BULK_REMOVE_UI_VERSION_MISSING
require_marker "$dockerfile" \
    'ENV CATTLE_RANCHER_SERVER_VERSION=v1.6.311' \
    SERVER_STORAGE_BULK_REMOVE_UI_RUNTIME_VERSION_MISSING
require_marker "$dockerfile" \
    'ENV PASTURESTACK_WEB_CONSOLE_PACKAGE=1.6.56-pasturestack.22' \
...
printf 'SERVER_STORAGE_BULK_REMOVE_UI_PATCH_OK release=v1.6.311 web_console=1.6.56-pasturestack.22 base=v1.6.310 locales=13 paging=10,25,50,all zero_count=visible runtime_digest_coordinates=0\n'
