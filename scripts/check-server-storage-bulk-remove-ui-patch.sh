...
require_marker "$dockerfile" \
    'selectedVolumeCount' \
    SERVER_STORAGE_BULK_REMOVE_UI_ZERO_COUNT_GATE_MISSING
require_marker "$dockerfile" \
...
require_marker "$dockerfile" \
    ...
    SERVER_STORAGE_BULK_REMOVE_UI_ZH_TW_FILTER_SET_MISSING
require_marker "$dockerfile" \
    'Retired duplicate storage-filter marker present' \
    SERVER_STORAGE_BULK_REMOVE_UI_RETIRED_FILTER_GATE_MISSING
...
if grep -Fq '"removable":"可勾選移除"' "$dockerfile" "$build_script"; then
    echo 'SERVER_STORAGE_BULK_REMOVE_UI_RETIRED_FILTER_TRANSLATION_PRESENT' >&2
    exit 1
fi

printf 'SERVER_STORAGE_BULK_REMOVE_UI_PATCH_OK release=v1.6.312 web_console=1.6.56-pasturestack.23 base=v1.6.311 locales=13 filters=all,active,detached paging=10,25,50,all zero_count=visible runtime_digest_coordinates=0\n'
