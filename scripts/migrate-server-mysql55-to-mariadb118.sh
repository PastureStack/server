...
NEW_IMAGE="${NEW_IMAGE:-ghcr.io/pasturestack/server:v1.6.272}"
...
RC16_AGENT_IMAGE="${RC16_AGENT_IMAGE:-ghcr.io/pasturestack/node-agent:v1.2.30@sha256:5310b748fc52bcd87fdeaa2285f424a07ec13c9b41639692eef96bda53ac8277}"
...
RC16_ARTIFACT_BASE_URL="${RC16_ARTIFACT_BASE_URL:-https://github.com/PastureStack/server/releases/download/v1.6.272}"
...
resolve_catalog_json() {
    ...
    CATALOG_JSON='{"catalogs":{"pasturestack":{"url":"https://github.com/PastureStack/catalog-templates.git","branch":"main","pinnedCommit":"d660aac76131a40f0c907c03133f7920e8ffbd9c"}}}'
    validate_catalog_json
...
select name,value from setting where name in ('bootstrap.required.image','agent.package.python.agent.url','agent.package.host.api.url','catalog.url','lb.instance.image','lb.instance.image.uuid','api.security.enabled','api.auth.provider.configured') order by name;
SQL
