...
image=${IMAGE:-pasturestack-validation/server:v1.6.285}
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.catalog-route53-dns-sync-patch \
    server
...
test "$(grep -Fxc "CATTLE_RANCHER_SERVER_VERSION=v1.6.285" \
    <<<"$image_environment")" = 1
...
printf 'SERVER_CATALOG_ROUTE53_DNS_SYNC_PATCH_IMAGE_OK image=%s revision=%s catalog_commit=%s source_date_epoch=%s\n' \
