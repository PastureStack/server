...
web_console_release_tag=${WEB_CONSOLE_RELEASE_TAG:-v1.6.56-pasturestack.20}
web_console_artifact=${WEB_CONSOLE_ARTIFACT:-web-console-1.6.56-pasturestack.20.tar.gz}
...
image=${IMAGE:-pasturestack-validation/server:v1.6.309}
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.storage-cleanup-ui-patch \
    server
...
printf 'SERVER_STORAGE_CLEANUP_UI_PATCH_IMAGE_OK image=%s revision=%s web_console_commit=%s\n' \
