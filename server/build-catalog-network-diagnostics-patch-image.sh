...
image=${IMAGE:-pasturestack-validation/server:v1.6.287}
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.catalog-network-diagnostics-patch \
    server
...
test "$(grep -Fxc "CATTLE_RANCHER_SERVER_VERSION=v1.6.287" \
    <<<"$image_environment")" = 1
...
printf 'SERVER_CATALOG_NETWORK_DIAGNOSTICS_PATCH_IMAGE_OK image=%s revision=%s catalog_commit=%s source_date_epoch=%s\n' \
