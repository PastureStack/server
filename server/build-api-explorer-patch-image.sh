...
orchestration_engine_release_tag=${ORCHESTRATION_ENGINE_RELEASE_TAG:-v0.183.308}
orchestration_engine_artifact=${ORCHESTRATION_ENGINE_ARTIFACT:-orchestration-engine-0.183.308.jar}
orchestration_engine_artifact_sha256=${ORCHESTRATION_ENGINE_ARTIFACT_SHA256:-9535af2c2ad3635912f3cda32699c0756311e3d0fe7eeff34b52dc83a1e20275}
orchestration_engine_commit=${ORCHESTRATION_ENGINE_COMMIT:-299ecf9252671db336aa6fb84adb7407786857e1}
...
for marker in \
    ...
    CATTLE_CATTLE_VERSION=v0.183.308 \
...
docker run --rm --entrypoint bash "$image" -lc '
    ...
    resources_jar=$(find "${web_root}/WEB-INF/lib" -maxdepth 1 -type f \
        -name "cattle-resources-0.183.308.jar" -print -quit)
...
docker run --rm --entrypoint bash "$image" -lc '
    ...
    unzip -p /usr/share/cattle/cattle.jar META-INF/MANIFEST.MF |
        tr -d "\r" |
        grep -Fx "Implementation-Version: 0.183.308" >/dev/null
