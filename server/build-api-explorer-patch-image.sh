...
orchestration_engine_release_tag=${ORCHESTRATION_ENGINE_RELEASE_TAG:-v0.183.315}
orchestration_engine_artifact=${ORCHESTRATION_ENGINE_ARTIFACT:-orchestration-engine-0.183.315.jar}
orchestration_engine_artifact_sha256=${ORCHESTRATION_ENGINE_ARTIFACT_SHA256:-8d92bcf24ff26340f7e92013408d46c81377965d2f9ef773936ca0118895ef8d}
orchestration_engine_commit=${ORCHESTRATION_ENGINE_COMMIT:-9faa58a7bf565999a9d219200867996a85eb8a2d}
...
image=${IMAGE:-pasturestack-validation/server:v1.6.456}
build_options=()
...
for marker in \
    CATTLE_RANCHER_SERVER_VERSION=v1.6.456 \
    CATTLE_API_UI_VERSION=1.1.18 \
    CATTLE_CATTLE_VERSION=v0.183.315 \
...
docker run --rm --entrypoint bash "$image" -lc '
    ...
    resources_jar=$(find "${web_root}/WEB-INF/lib" -maxdepth 1 -type f \
        -name "cattle-resources-0.183.315.jar" -print -quit)
...
docker run --rm --entrypoint bash "$image" -lc '
    ...
    unzip -p /usr/share/cattle/cattle.jar META-INF/MANIFEST.MF |
        tr -d "\r" |
        grep -Fx "Implementation-Version: 0.183.315" >/dev/null
