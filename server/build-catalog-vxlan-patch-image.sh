...
image=${IMAGE:-pasturestack-validation/server:v1.6.280}
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.catalog-vxlan-patch \
    server
...
test "$(docker image inspect "$image" \
    ...
    grep -Fxc \
        'PASTURESTACK_CATALOG_COMMIT=23ee314045ccbd2445748c90567dbe24b6ab801e')" = 1
test "$(docker image inspect "$image" \
    --format '{{range .Config.Env}}{{println .}}{{end}}' |
    grep -Fxc \
        'PASTURESTACK_WEB_CONSOLE_COMMIT=2dd5e5b0154ddbb8e41ef2887fa786b93e74827b')" = 1

printf 'SERVER_CATALOG_VXLAN_PATCH_IMAGE_OK image=%s revision=%s source_date_epoch=%s\n' \
