...
authentication_service_release_tag=${AUTHENTICATION_SERVICE_RELEASE_TAG:-v0.2.3}
authentication_service_artifact=${AUTHENTICATION_SERVICE_ARTIFACT:-authentication-service-0.2.3-linux-amd64.tar.xz}
...
web_console_release_tag=${WEB_CONSOLE_RELEASE_TAG:-v1.6.56-pasturestack.27}
web_console_artifact=${WEB_CONSOLE_ARTIFACT:-web-console-1.6.56-pasturestack.27.tar.gz}
...
image=${IMAGE:-pasturestack-validation/server:v1.6.315}
build_options=()
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.oidc-recovery-patch \
    server
...
test "$(docker image inspect "$image" \
    ...
    ghcr.io/pasturestack/server:v1.6.314

image_environment=$(docker image inspect "$image" \
    --format '{{range .Config.Env}}{{println .}}{{end}}')
for marker in \
    CATTLE_RANCHER_SERVER_VERSION=v1.6.315 \
    PASTURESTACK_DOCKER_SUPPORT_POLICY=2026-07-27 \
    PASTURESTACK_AUTHENTICATION_SERVICE_VERSION=0.2.3 \
    ...
    PASTURESTACK_WEB_CONSOLE_PACKAGE=1.6.56-pasturestack.27 \
...
docker run --rm --entrypoint bash "$image" -lc '
    ...
    /usr/bin/authentication-service.real --version | grep -F "0.2.3" >/dev/null
...
printf 'SERVER_OIDC_RECOVERY_PATCH_IMAGE_OK image=%s revision=%s authentication_service_commit=%s web_console_commit=%s\n' \
