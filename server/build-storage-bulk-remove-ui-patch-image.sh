...
web_console_release_tag=${WEB_CONSOLE_RELEASE_TAG:-v1.6.56-pasturestack.23}
web_console_artifact=${WEB_CONSOLE_ARTIFACT:-web-console-1.6.56-pasturestack.23.tar.gz}
...
image=${IMAGE:-pasturestack-validation/server:v1.6.312}
...
docker run --rm --entrypoint bash "$image" -lc '
    ...
    ! grep -F "removable" "${ui_entry}" >/dev/null
    ! grep -R -F "\"removable\":" \
