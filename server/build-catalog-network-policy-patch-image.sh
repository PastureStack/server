...
image=${IMAGE:-pasturestack-validation/server:v1.6.288}
catalog_commit=b3f412fb07af983553ec9c1b61f6db00de3d9742
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.catalog-network-policy-patch \
    server
...
test "$(grep -Fxc "CATTLE_RANCHER_SERVER_VERSION=v1.6.288" \
    <<<"$image_environment")" = 1
...
printf 'SERVER_CATALOG_NETWORK_POLICY_PATCH_IMAGE_OK image=%s revision=%s catalog_commit=%s source_date_epoch=%s\n' \
