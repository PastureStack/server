...
image=${IMAGE:-pasturestack-validation/server:v1.6.281}
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.catalog-container-schedule-patch \
    server
...
test "$(docker image inspect "$image" \
    ...
    grep -Fxc \
        'PASTURESTACK_CATALOG_COMMIT=8e4caa289895855df9706ebf625f401da4ca2e5b')" = 1
test "$(docker image inspect "$image" \
    --format '{{range .Config.Env}}{{println .}}{{end}}' |
    grep -Fxc \
        'PASTURESTACK_WEB_CONSOLE_COMMIT=2dd5e5b0154ddbb8e41ef2887fa786b93e74827b')" = 1

printf 'SERVER_CATALOG_CONTAINER_SCHEDULE_PATCH_IMAGE_OK image=%s revision=%s source_date_epoch=%s\n' \
