...
orchestration_engine_release_tag=${ORCHESTRATION_ENGINE_RELEASE_TAG:-v0.183.270}
orchestration_engine_artifact=${ORCHESTRATION_ENGINE_ARTIFACT:-orchestration-engine-0.183.270.jar}
...
web_console_release_tag=${WEB_CONSOLE_RELEASE_TAG:-v1.6.56-pasturestack.28}
web_console_artifact=${WEB_CONSOLE_ARTIFACT:-web-console-1.6.56-pasturestack.28.tar.gz}
...
image=${IMAGE:-pasturestack-validation/server:v1.6.316}
build_options=()
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.identity-security-patch \
    server
...
test "$(docker image inspect "$image" \
    ...
    ghcr.io/pasturestack/server:v1.6.315

image_environment=$(docker image inspect "$image" \
    --format '{{range .Config.Env}}{{println .}}{{end}}')
for marker in \
    CATTLE_RANCHER_SERVER_VERSION=v1.6.316 \
    CATTLE_CATTLE_VERSION=v0.183.270 \
    ...
    PASTURESTACK_WEB_CONSOLE_PACKAGE=1.6.56-pasturestack.28 \
...
docker run --rm --entrypoint bash "$image" -lc '
    ...
    unzip -p /usr/share/cattle/cattle.jar META-INF/MANIFEST.MF |
      tr -d "\r" |
      grep -Fx "Implementation-Version: 0.183.270" >/dev/null
...
printf 'SERVER_IDENTITY_SECURITY_PATCH_IMAGE_OK image=%s revision=%s engine_commit=%s authentication_service_commit=%s web_console_commit=%s\n' \
