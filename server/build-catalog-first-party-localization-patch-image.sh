...
web_console_sha256=b080f2299aed008e95071ed754163b3761d5399b95e0e2532e7bd2dd1bacb6ba
...
test "$(grep -Fxc "CATTLE_RANCHER_SERVER_VERSION=v1.6.296" \
    <<<"$image_environment")" = 1
...
test "$(grep -Fxc "PASTURESTACK_WEB_CONSOLE_COMMIT=${web_console_commit}" \
    <<<"$image_environment")" = 1
test "$(grep -Fxc \
    'PASTURESTACK_WEB_CONSOLE_PACKAGE=1.6.56-pasturestack.9' \
    <<<"$image_environment")" = 1
test "$(grep -Fxc \
