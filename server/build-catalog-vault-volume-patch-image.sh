...
image=${IMAGE:-pasturestack-validation/server:v1.6.291}
catalog_commit=4205e1d3b4fec7691c9e5531375f5268c80924da
...
docker buildx build \
    ...
    --tag "$image" \
    --file server/Dockerfile.catalog-vault-volume-patch \
    server
...
test "$(grep -Fxc "CATTLE_RANCHER_SERVER_VERSION=v1.6.291" \
    <<<"$image_environment")" = 1
...
printf 'SERVER_CATALOG_VAULT_VOLUME_PATCH_IMAGE_OK image=%s revision=%s catalog_commit=%s source_date_epoch=%s\n' \
